classdef Delays < WaveProp
	...
	methods  
        ...
        % from scratch if you get irritated.
			...
			for ff = string(fieldnames(mea)')
				switch lower(ff)
					...
					case 'samplingrate'
						...
						if isempty(D.AllTime), D.AllTime = 1:size(signal, 1)' / mea.(ff); end
                    case 'patient'
                        D.Patient = mea.(ff);
                    case 'seizure'
                        D.Seizure = mea.(ff);
			...
			if isempty(t0), t0 = mean(D.AllTime); end
            ...
            % Get the data window
            t_inds = abs(D.AllTime - D.t0) <= D.HalfWin;
			D.T = range(D.AllTime(t_inds));
