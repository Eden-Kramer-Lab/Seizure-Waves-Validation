...
%
%INPUTS
%  scm      = model parameters (SCM class)
%  time_end = total time of simulation (/s).
%  IC       = structure with initial conditions for model
%             variables. (SCMState class)
%  fig      = (optional) a figure where simulation progress is displayed
%                      
%OUTPUTS
...
%---------------------------------------------------------------------
function [NP, time, last, fig] = seizing_cortical_field(scm, time_end, IC, fig)
    % [NP, time, last, fig] = seizing_cortical_field(scm, time_end, IC, fig)
...
% Computed fixed parameters
phi_ee_sc = scm.phi_ee_sc;
phi_ei_sc = scm.phi_ei_sc;

% noise-amplitude coefficients for subcortical flux (note 1/sqrt(dt) factor)
B_ee = scm.noise_sf .* sqrt(scm.noise_sc .* phi_ee_sc / dt);
B_ei = scm.noise_sf .* sqrt(scm.noise_sc .* phi_ei_sc / dt);
	...
	%%%% E-to-E %%%%
		new.F_ee = last.F_ee ...
			+ dt * scm.gamma_e.^2 * ( ...
				...
				+ scm.noise_sc .* phi_ee_sc ... % subcortical (tonic)
	...
	%%%% E-to-I %%%%
		new.F_ei = last.F_ei ...
			+ dt * scm.gamma_e.^2 * ( ...
				...
				+ scm.noise_sc .* phi_ei_sc ...    %subcortical (tonic)
	...
	function visualize
		if scm.visualization_rate > 0
			...
			if diff(floor((time(ii) - [dt 0]) * scm.visualization_rate))
                ...
                imwrite(im, ...
                        sprintf('SCM/%s/vids/%s_%d/%0.4f.png', ...
                                scm.label, scm.label, scm.sim_num, time(ii)));
...
function fig = create_fig_(out_vars, grid_size, addyNP, ~)
	...
	stop_at = 'seizing_cortical_field>get_electrode_values';
	titles = out_vars;
	N = numel(titles);
	fig.fig = figure;
	fig.quit_early = false;
	% Create 'Debug' pushbutton in figure window
